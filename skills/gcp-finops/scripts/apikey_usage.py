...
def fetch_and_display(project_id, breakdown_by_product=False, allowed_types=None):
    ...
    for r_name, r_config in ranges.items():
        ...
        for series in results:
            ...
            if cred_type not in allowed_types and "all" not in allowed_types:
                continue
    ...
    print(f"\nGenAI usage for Project: {project_id} (Filtered by: {', '.join(allowed_types)})")
    print("=" * 180)
    ...
    for c_type in sorted(grouped_rows.keys()):
        ...
        print("-" * 180)
        print(f"{'Cost (24h)':>10} | {'Cost (30d)':>10} | {'24h':<8} | {'30d':<10} | {'Identifier (ID)':<40} | {'Identity':<30}" + (" | Service" if breakdown_by_product else ""))
        print("-" * 180)
...
def main():
    ...
    fetch_and_display(args.project, args.breakdown_by_product, args.credential_types.split(","))
